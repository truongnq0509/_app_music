        ...
        <div className={cx('wrapper')}>
            <div className={cx('info')}>
                <div className={cx('info-left')}>
                    {isLoading ? (
                        ...
                        <Image src={artist?.thumbnailM} alt="thumnail" />
                    )}
                </div>
                <div className={cx('info-right')}>
                    <h1 className={cx('info-right__name')}>
                        {isLoading ? (
                            <Skeleton sx={{ fontSize: '5rem', width: '50%' }} />
                        ) : (
                            <>
                    ...
                    <div className={cx('info-right__date')}>
                        {artist?.birthday && (
                            <span>
                                {isLoading ? (
                                    <Skeleton sx={{ fontSize: '1.8rem', width: '200px' }} />
                                ) : (
                    ...
                    <div className={cx('info-right__biography')}>
                        {isLoading ? (
                            ...
                            <>
                                <span>
                                    {artist?.biography
                                        ? artist?.biography?.replace(/<br>/g, '').length > 500
                                            ? `${artist?.biography?.replace(/<br>/g, '').slice(0, 500)} . . . `
                                            : `${artist?.biography?.replace(/<br>/g, '')}`
                                        : artist?.sortBiography?.replace(/<br>/g, '').length > 160
                                        ? `${artist?.sortBiography?.replace(/<br>/g, '').slice(0, 160)} . . . `
                                        : `${artist?.sortBiography?.replace(/<br>/g, '')}`}
                                </span>
                                {artist?.biography?.length > 500 && (
                                    <span
                                        onClick={() => setIsModal((prev) => !prev)}
                                        className={cx('info-right__more')}
                                    >
                                        {' '}
                                        Xem thêm{' '}
                                    </span>
                    ...
                    <div className={cx('info-right__button')}>
                        ...
                        {isLoading ? (
                            <Skeleton sx={{ fontSize: '1.8rem', width: '100px', height: '50px' }} />
                        ) : (
                            <Button icon={<UserIcon w="1.6rem" h="1.6rem" />} title="Follow" large />
                        )}
                    </div>
                </div>
            </div>
            ...
            >
                <div className={cx('modal__content')}>
                    ...
                    <div className={cx('modal__body')}>
                        <div className={cx('modal__biography')}>{`${artist?.biography?.replace(/<br>/g, '')}`}</div>
                    </div>
                </div>
            </div>
            ...
            <div className={cx('section')}>
                    ...
                    <div className={cx('section-left')}>
                        <div className={cx('section-left__header')}>
                            <h3 className={cx('section-left__text')}>
                                {isLoading ? (
                                    <Skeleton sx={{ fontSize: '3.6rem', width: '200px' }} />
                                ) : (
                                    `Bài Hát Nổi Bật`
                                )}
                            </h3>
                        ...
                        <div className={cx('section-left__body')}>
                            <div className={cx('section-left__thumbail')}>
                                {isLoading ? (
                                    <Skeleton
                                        variant="rectangular"
                                        sx={{ bgcolor: 'rgba(51, 55, 59, 0.37)', width: '100%', height: '100%' }}
                                    />
                                ) : (
                                    <Image src={artist?.topAlbum?.thumbnailM} alt="thumnail" />
                                )}
                            </div>
                            <div className={cx('section-left__info')}>
                                <span className={cx('section-left__type')}>
                                    {isLoading ? (
                                        <Skeleton
                                            sx={{ fontSize: '1.8rem', width: '20%', bgcolor: 'rgba(51, 55, 59, 0.37)' }}
                                        />
                                    ) : (
                                        artist?.topAlbum?.textType
                                    )}
                                </span>
                                <div className={cx('section-left__box')}>
                                    <h4 className={cx('section-left__title')}>
                                        {isLoading ? (
                                            <Skeleton
                                                sx={{
                                                    fontSize: '1.8rem',
                                                    maxWidth: '200px',
                                                    bgcolor: 'rgba(51, 55, 59, 0.37)',
                                                }}
                                            />
                                        ) : (
                                    ...
                                    <span className={cx('section-right__date')}>
                                        {isLoading ? (
                                            <Skeleton
                                                sx={{
                                                    fontSize: '1.8rem',
                ...
                >
                    ...
                    <div className={cx('section-right__body')}>
                        {isLoading ? (
                            <SongSkeleton limit={3} />
                        ) : (
                            artist?.sections?.[0]?.items
                                ?.filter((item, index) => index < 3)
                                ?.map((song) => <Song key={song.encodeId} song={song} />)
                        )}
                    </div>
                </div>
            </div>
            ...
            {isLoading ? (
                <>
                    ...
                    <SectionsSkeleton limit={1} isVideo />
                    <SectionsSkeleton limit={1} isArtist />
                </>
            ) : (
                artist?.sections
                    ?.filter((item) => item?.sectionType !== 'song')
                    ?.map((section, index) => (
                        <Sections
                            key={index}
                            data={section}
